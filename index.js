document.addEventListener('DOMContentLoaded', function() {
  ...
  let eventDate = new Date( yearOfTheEvent, 04, 23 )
  const isItFebruary4th = currentTime.getMonth() === 04 && currentTime.getDate() === 23

  function countdown() {
    const now = new Date()
    
    if (now > eventDate) {
      eventDate = new Date( yearOfTheEvent + 1, 04, 23 )
    } else if ( now.getFullYear() === eventDate.getFullYear() + 1 ) {
      eventDate = new Date( now.getFullYear(), 04, 23 )
    }
